...
export default async function PaginaEditMaestra({ 
    ...
    return (
      <main className="min-h-screen bg-slate-50 p-4 flex flex-col items-center">
        <div className="w-full max-w-md">
          ...
          <form action={guardarCambios} className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-8 space-y-4">
            <input type="hidden" name="id" value={id} />
            
            <header className="flex items-center gap-4 mb-2">
              <div className="bg-blue-600 p-3 rounded-2xl text-white shadow-lg"><Monitor size={24} /></div>
              <h1 className="text-xl font-black text-slate-800 uppercase italic">Ficha Técnica</h1>
            </header>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input name="numero" defaultValue={data.numero_inventario} className="w-full p-3 bg-slate-50 rounded-xl border font-bold text-slate-700 outline-none focus:border-blue-500" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase ml-2 flex items-center gap-1"><Shield size={10}/> Resp. Área</label>
                  <input name="jefe_area" defaultValue={data.responsable_area} placeholder="Jefe de depto." className="w-full p-3 bg-slate-50 rounded-xl border font-bold text-slate-700 outline-none focus:border-blue-500" />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <input name="responsable" defaultValue={data.responsable_inventario} className="w-full p-3 bg-slate-50 rounded-xl border font-bold text-slate-700 outline-none focus:border-blue-500" />
              </div>

              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase ml-2">Marca y Modelo</label>
                <input name="marca" defaultValue={data.marca_modelo} className="w-full p-3 bg-slate-50 rounded-xl border font-bold text-slate-700 outline-none focus:border-blue-500" />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input name="mouse_teclado" defaultValue={data.mouse_teclado} placeholder="Mouse/Teclado mod." className="w-full p-3 bg-slate-50 rounded-xl border font-bold text-slate-700 outline-none focus:border-blue-500" />
              </div>
  ...
  const computadoras = allResult.rows as any[];
